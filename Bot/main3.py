...
def openfromfile():
    ...
    try:
        ...
        for i in range(len(lines)//6):
            ...
            accounts[-1].add_handler(MessageHandler(answ_thread, filters.private))
            select_id_plus()
...
async def answ_thread(client, message):
    print(message.text)
    await Thread(target=answer, args=(client, message)).start()
...
def main():
    ...
    for i in range(2,4):
        accounts[i].start()
        print("Bot " + str(i) + " is running!")
    print("All bots is running!")
    idle()
    # mybot.stop()
    for i in range(2,4):
        accounts[i].stop()
